...
public class DBHelper extends SQLiteOpenHelper {
    static Integer dbVersion = 6;
    private BaseDAO[] baseDao = new BaseDAO[]{
            new TbUser(null),
            new TbWilayah(null),
            new TbBlok(null),
            new TbKonfigurasi(null),
            new TbGantiMeter(null),
    };
    ...
    public DBHelper(Context context) {
        super(context, "PDAMGANTIMETER.db", null, dbVersion);
    }
    ...
    public void onCreate(SQLiteDatabase db) {
        for (BaseDAO domain : baseDao) {
            String CREATE_TABLE = "CREATE TABLE IF NOT EXISTS " + DBUtils.getTableName(domain) + " (" + DBUtils.getCloumns(domain) + ")";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        for (BaseDAO domain : baseDao) {
            db.execSQL("DROP TABLE IF EXISTS " + DBUtils.getTableName(domain));
        }
        onCreate(db);
    }
    ...
    public void save(BaseDAO domain) {
        ...
        // Inserting Row
        db.insert(DBUtils.getTableName(domain), null, values);
    ...
    public int update(BaseDAO domain) {
        ...
        // updating row
        int a = db.update(DBUtils.getTableName(domain), values, keyid,
    ...
    public int delete(BaseDAO domain) {
        ...
        int a = 0;
        if(params.isEmpty()){
            a= db.delete(DBUtils.getTableName(domain), null, null);
        }else{
            a= db.delete(DBUtils.getTableName(domain), keyid,
                    params.toArray(new String[]{}));
    ...
    public Cursor getDataByPrimaryKey(BaseDAO domain){
        ...
        String keyid="";
        String tableName = DBUtils.getTableName(domain);
    ...
    public Cursor getAllData(BaseDAO domain) {
        // Select All Query
        String selectQuery = "SELECT "+DBUtils.getCloumns(domain)+" FROM " + DBUtils.getTableName(domain);
        SQLiteDatabase db = this.getWritableDatabase();
