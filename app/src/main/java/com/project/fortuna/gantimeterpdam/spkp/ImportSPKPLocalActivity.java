...
public class ImportSPKPLocalActivity extends AppCompatActivity {
    ...
    private File path = new File(Environment.getExternalStorageDirectory().toString() + "/GantiMeter/CSV/");
    private String chosenFile;
    private List<TbGantiMeter> tPemasangan = new ArrayList<TbGantiMeter>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_import_spkplocal);

        toolbar = (Toolbar) findViewById(R.id.toolbar);
        toolbar.setTitle("Import Local");
    ...
    public class ButtonImportClickHandler implements View.OnClickListener {
        public void onClick(View view) {
            ...
            if (!tPemasangan.isEmpty()) {
                boolean flag = false;
                try {
                    for (TbGantiMeter tm : tPemasangan) {
    ...
    public void importFileCSV() throws Exception {
        ...
        CSVReader csvParser = new CSVReaderBuilder(reader).strategy(strategy)
        ...
        for (String[] sData : data) {
            ...
            {
                System.out.println("[" + sData[14] + "]["+ pref.getString("userid", "") + "");
                if (sData[14].equals(pref.getString("userid", ""))) {
                    TbGantiMeter tm = new TbGantiMeter(getApplicationContext());
                    tm.NO_SPKP = sData[0];
                    tm.NO_BAP = sData[1];
                    tm.KODE_PELANGGAN = sData[2];
                    tm.NAMA_PELANGGAN = sData[3];
                    tm.ALAMAT_PELANGGAN = sData[4];
                    tm.KD_WILAYAH = sData[5];
                    tm.KD_JALAN = sData[6];
                    tm.KD_STAND_METER_BARU = sData[7];
                    tm.MERK_METER_BARU = sData[8];
                    tm.ANGKA_METER_LAMA = sData[9];
                    tm.GPS_LAT_PELANGGAN = sData[10];
                    tm.GPS_LONG_PELANGGAN = sData[11];
                    tm.MERK_METER_LAMA = sData[12];
                    tm.KD_STAND_METER_LAMA = sData[13];
                    tm.USER_ID = sData[14];
                    tm.S_PASANG = "-1";
                    ...
                    // Util.showmsg(ImportSPKPFromLocal.this, "Peringatan :",
                    // "User ID Tidak Sesuai, Data Tidak Dapat di Import");
                }
            } else {
                flag = true;
            }
        ...
        if (flag) {
            Util.showmsg(ImportSPKPLocalActivity.this, "Peringatan :",
                    "Data Kolom Tidak Sesuai Sebanyak " + String.valueOf(i + 1)
                            + " Baris");
        }

        ListView l = (ListView) findViewById(R.id.id_import_list_view);
        l.setAdapter(new ImportSPKPAdapter(this, tPemasangan));

        Util.showmsg((Context) this, "Peringatan :", "Data yang di Import : "
                + String.valueOf(tPemasangan.size()));
    }

    @SuppressWarnings({ "static-access", "unused" })
    private void launchBarDialog() {
        barProgressDialog = new ProgressDialog(ImportSPKPLocalActivity.this);
    ...
    private void loadFilesOnPath() {
        ...
        if (path.exists()) {
            ...
            new AlertDialog.Builder(this)
                    .setTitle("Warning")
                    .setMessage("Path [SDCARD]/Ganti Meter/CSV not found.")
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        // File picked
                            ...
                            try {
                                ...
                                Util.showmsg(ImportSPKPLocalActivity.this,
